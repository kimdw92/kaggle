# Category classifier
# 파이토치 기반의 모델 생성 + loss 계산
# - 텍스트 인코더 : BERT 모델(허깅페이스)
# - 이미지 인코더 : 선형 모델



import torch # 파이토치 패키지
import torch.nn as nn # 자주 사용하는 torch.nn 패키지를 nn으로 멸명
# 허깅페이스의 트랜스포머 패키지
...
class CateClassifier(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        def get_cls(target_size=1):
            return nn.Sequential(nn.Linear(cfg.hidden_size*2, cfg.hidden_size),
    ...
    def forward(self, token_ids, token_mask, token_types, img_feat, label=None):
        ...
        # 전처리된 상품명을 한의 텍스트벡터(text_vec)로 변환
        # 변환 튜플(시퀀스 아웃풋, pooled 아웃풋) 중 시퀀스 아웃풋만 참조
        ...
        # 이미지벡터와 텍스트벡터를 직렬연결하여 결합벡터 생성
        comb_vec = torch.cat([text_vec, img_vec], 1)
        
        # 결합된 벡터로 카테고리 확률분포 예측
